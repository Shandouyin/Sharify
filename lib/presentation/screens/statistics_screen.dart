...
class StatisticsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Map<String, int> popularGenres =
        dataService.getPopularGenresFromTop3s();
    final Map<String, int> popularArtists =
        dataService.getPopularArtistsFromTop3s();
    final List<MusicModel> top10Tracks =
        dataService.getTop10PopularTracksFromTop3s();

    // Si pas de données des Top3, fallback sur les données statiques
    final Map<String, int> fallbackGenres =
        popularGenres.isEmpty ? dataService.getPopularGenres() : popularGenres;
    final Map<String, int> fallbackArtists = popularArtists.isEmpty
        ? dataService.getPopularArtists()
        : popularArtists;
    final List<MusicModel> fallbackTracks =
        top10Tracks.isEmpty ? dataService.getTop10PopularTracks() : top10Tracks;
  ...
  Widget _buildTopTracksSection(
      BuildContext context, List<MusicModel> topTracks) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Padding(
        ...
        if (topTracks.isEmpty)
          EmptyStateWidget(
            ...
            title: 'Aucune musique populaire',
